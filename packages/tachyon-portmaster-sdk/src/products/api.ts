...
export const createProduct = (c: ApiClient, body: ProductCreateRequest): Promise<Product> =>
  wire(c, { method: 'POST', path: '/v1/products', body, encode: encProductCreate, decode: decProduct });

export const updateProduct = (c: ApiClient, id: string, body: ProductUpdateRequest): Promise<Product> =>
  wire(c, {
    method: 'PUT',
    path: `/v1/products/${id}`,
    body,
