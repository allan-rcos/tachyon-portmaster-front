...
that run close to the metal. It is *not* a frontend or backend specialist, and *not* a UI shop;
...
The `ui_kits/console/` example is *one* product surface (a bare-metal compute console) — it shows
the system in use, but the brand itself is broader than any single interface.

**Name:** *Tachyon* — after the hypothetical faster-than-light particle: pure **speed** with real
engineering depth, and it reads like a serious infra/dev company. **Confirmed** by the user.
The wordmark lights the **"on"** in gold (*powered on / connected*) — `Tachy` in paper, `on` in
...
- `uploads/final.svg` — the master logo artwork (1200×1200). **Note:** the source file is a
  ...
  *Logo & assets* below. A clean multi-color vector re-export from the user would be ideal.
...
- **The script accent (Parisienne)** carries the *emotional* line only — a tagline or hero
  flourish like *"bare metal, alive"*. Never use it for body, labels, or anything functional.
...
- **Color** — Primary **gold `#ECBA28`** for action and brand accent. Auxiliaries: **sage
  ...
  plus a warm **paper** cream. A clay-red `#CF4F2C` is *derived* for danger only (not a brand
...
- **System:** [**Lucide**](https://lucide.dev) (`unpkg.com/lucide`) — clean 24×24, **2px** stroke,
  ...
  *Lucide is a substitution* — no proprietary icon set was supplied. Flag if the user has their own.
...
Lockup: emblem + **Ubuntu** wordmark *"Tachy**on**"* — `Tachy` in paper/ink, the **"on" lit gold**
(`--gold-400`), evoking *powered on / connected*. Optional Parisienne tagline (*"faster than the
metal"*). Keep the emblem inside its coin; never crop it.

> **Positioning note:** Tachyon is a **software-development** company built on **performance &
> speed** — fast stacks and code, close to the metal — *not* a frontend/UI shop. The voice leans
...
- **Compiler namespace:** the generated bundle global is `window.CircuitaDesignSystem_12e0ad`, a
  frozen handle tied to this project's creation ID — it is *not* brand text and never appears in any
