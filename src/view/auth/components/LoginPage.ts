 ...
 */
export function LoginPage(props: LoginPageProps): TemplateResult {
  const { vm } = props;

  return html`<main class=${styles.page}>
    <section class=${styles.promo} aria-labelledby="promo-title">
      ${Brand()}
      <h1 id="promo-title" class=${styles.headline}>${vm.t.title}</h1>
      <p class=${styles.desc}>${vm.t.subtitle}</p>
      <p class=${styles.script}>${vm.t.script}</p>
    </section>

    <section class=${styles.formPanel} aria-label=${vm.t.title}>
      ${LoginForm({ vm })}
      <p class=${styles.hint}>Demo: <code>ana@portmaster.test</code> / <code>admin123</code></p>
    </section>
  </main>`;
}
