 ...
 */
export function OccupancyBreakdown(props: OccupancyBreakdownProps): JSX.Element {
  return (
    <ul class={styles.rows} aria-label={props.label}>
      <For each={props.rows}>
        {(row) => (
          <li class={styles.row} data-tone={row.tone}>
            ...
            <span class={styles.track}>
              <span class={styles.fill} style={{ width: `${row.share}%` }} />
            </span>
          </li>
        )}
      </For>
