 ...
 */
export function ErrorPage(props: ErrorPageProps): TemplateResult {
  const title = props.forbidden
    ? '403 — Acesso negado'
    : props.is404
      ? '404 — Página não encontrada'
      : '500 — Erro no servidor';

  const message = props.forbidden
  ...
  return html`<main class=${styles.page}>
    ...
    ${
      props.forbidden
        ? nothing
        : html`<p><a class=${styles.link} href="/painel">← Ir para o painel</a></p>`
    }
  </main>`;
}
