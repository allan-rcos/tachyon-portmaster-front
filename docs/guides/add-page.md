...
`src/view/<feature>/screens/MinhaRotaScreen.ts` — função stateless que recebe o
VM e devolve um `TemplateResult`:

```ts
export function MinhaRotaScreen(props: { vm: MinhaRotaVM }): TemplateResult {
...
export default function Page(pageContext: PageContext): PageView {
  const vm = createMinhaRotaVM(pageContext.data as MinhaRotaPageInput);
  return () => MinhaRotaScreen({ vm });
}
```

A fábrica roda **uma vez** por página; o thunk que ela devolve é o que o laço de
render reavalia. É isso que faz o estado do VM (formulário digitado, páginas já
trazidas pelo cursor) sobreviver aos re-renders. Construir o VM dentro do thunk o
recriaria zerado a cada tecla.

`pageContext` chega por **argumento** — não há `useData()` nem
`usePageContext()`.
